...
The script should be runnable from command line.
The python code shouldn't include the dictionaries of the plan's steps.
The python code shouldn't have an input argument of `plan`.
The python script should include all the input parameters as arguments to the script.
Parse the plan steps and for each step add its tool call.
If there are hardcoded values of the input parameters of the steps then replace it with an appropriate input argument with the hardcoded value as a default values of the argument.
The function calls of the steps shouldn't include hardcoded values in the parameters.
...
def execute_workflow(args):
    results = 
    res = LangIdentificationTransform()._run(input_folder=args.in_folder, output_folder=args.out_folder+"_langid")
    results["Ev1"] = parse_output(res)
    res = FilterTransform()._run(input_folder=results["Ev1"], output_folder=results["Ev1"]+"_filter", filter_criteria_list=args.filter_criteria_list)
    results["Ev2"] = parse_output(res)
    res = TokenizationTransform()._run(input_folder=results["Ev2"], output_folder=results["Ev2"]+"_token")
